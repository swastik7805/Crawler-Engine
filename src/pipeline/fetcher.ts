...
import { FetchResult } from "../types/pipeline.js";

const USER_AGENT =
  "TheChronicleBot/1.0 (Web3 search index; +https://auth-backend-fawn.vercel.app/bot)";
...
export async function fetchPage(
  ...
  try {
    const result = await pRetry(
      async () => {
        ...
        if (
          ...
          throw new AbortError(
            `Non-HTML content-type "${contentType}" at ${url}`,
          );
        }
      ...
      },
      {
        ...
        onFailedAttempt: (error) => {
          logger.warn(
            `Fetch attempt ${error.attemptNumber}/${MAX_RETRIES + 1} failed` +
    ...
    logger.error(
      `Fetch permanently failed for ${url}: ${(err as Error).message}`,
    );
    return null;
  }
}
